...
describe('daily reward wager gate', () => {
  ...
  it('defaults to exactly $10M wagered during the UTC day', async () => {
  ...
  it('refuses one unit short and allows the exact threshold', async () => {
    await assert.rejects(
      requireDailyRewardWager(new WagerClient(9_999_999n), config, 'user-1'),
      (error: { code?: string }) => error.code === 'STREAK_WAGER_REQUIRED',
    );
    const exact = await requireDailyRewardWager(
      new WagerClient(10_000_000n),
      config,
      'user-1',
    );
